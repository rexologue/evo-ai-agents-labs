# db-mcp

MCP сервер для работы с профилями компаний и результатами поиска закупок.

## 🚀 Возможности
- `create_company_profile` — сохраняет профиль компании в PostgreSQL.
- `get_company_profile` — возвращает профиль компании по UUID.
- `list_company_profiles` — выводит список профилей с поиском и пагинацией.

## 🔧 Переменные окружения

Все переменные должны быть заданы в `.env` (см. `.env.example`).

### Обязательные
- `DB_HOST` — хост PostgreSQL
- `DB_PORT` — порт PostgreSQL
- `DB_NAME` — имя базы данных
- `DB_USER` — пользователь базы данных
- `DB_PASSWORD` — пароль пользователя
- `DB_MCP_HOST` — хост HTTP транспорта MCP
- `DB_MCP_PORT` — порт HTTP транспорта MCP

## 🚀 Запуск
```bash
docker buildx build --platform linux/amd64 -t db-mcp -f Dockerfile ..
docker run --rm --network host --env-file .env db-mcp
```

После запуска сервер будет доступен по адресу из `.env` (`DB_MCP_HOST` + `DB_MCP_PORT`)
с транспортом `streamable-http`.

## 📖 Использование инструментов
- `create_company_profile(profile)` — сохраняет профиль и возвращает запись из БД.
- `get_company_profile(company_id)` — отдает профиль по UUID.
- `list_company_profiles(query?, limit?, offset?)` — список профилей с поиском.

Полное описание аргументов доступно в `mcp_tools.json`

## Выгрузка на Cloud.ru
```bash
docker tag db-mcp:latest your-artifact-registry.cloud.ru/db-mcp-repo:v1.0.0
docker push your-artifact-registry.cloud.ru/db-mcp-repo:v1.0.0
```
